...
| 文件 | 行数 | 功能 |
|------|------|------|
| `cortexterm/state.py` | 280 | Store 状态管理 + AppState |
| `cortexterm/cost_tracker.py` | 280 | 费用追踪 + 使用统计 |
| `cortexterm/tooling.py` | 扩展 | Tool Protocol + Metadata |
...
```python
from cortexterm.state import create_app_store, format_app_state_summary
...
```python
from cortexterm.tooling import Tool, ToolMetadata, ToolCapability
...
```python
# tty_app.py 中已添加：
from cortexterm.state import AppState, Store, create_app_store
from cortexterm.cost_tracker import CostTracker
...
编辑 `cortexterm/cli_commands.py`，添加：
...
编辑 `cortexterm/agent_loop.py`，在 API 调用后添加：
...
编辑 `cortexterm/tty_app.py` 的 `on_tool_result` 回调：
...
| 维度 | Claude Code | CortexTerm (之前) | CortexTerm (现在) |
|------|-------------|----------------------|----------------------|
