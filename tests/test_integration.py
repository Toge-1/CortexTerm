"""End-to-end integration tests for CortexTerm.
...
from cortexterm.tooling import ToolContext, ToolRegistry, ToolDefinition, ToolResult
from cortexterm.tools import create_default_tool_registry
from cortexterm.types import AgentStep, ChatMessage
from cortexterm.context_manager import ContextManager
from cortexterm.session import SessionData, save_session, load_session, list_sessions
from cortexterm.config import load_effective_settings, CORTEXTERM_DIR
from cortexterm.prompt import build_system_prompt
from cortexterm.tui.types import TranscriptEntry, _create_transcript_entry, _recycle_transcript_entry
...
class TestAgentLoopIntegration:
    ...
    def test_edit_file_via_agent(
        ...
        system_messages.append({
            "role": "user",
            "content": "/edit hello.txt::Hello, world!::Hello, CortexTerm!",
        })
        ...
        assert "Hello, CortexTerm!" in content
...
class TestSessionIntegration:
    ...
    def test_session_save_load_roundtrip(self, tmp_workspace):
        ...
        # Cleanup
        session_path = CORTEXTERM_DIR / "sessions" / "test-integration-001.json"
    ...
    def test_session_listing(self, tmp_workspace):
        ...
        # Cleanup
        for i in range(3):
            path = CORTEXTERM_DIR / "sessions" / f"test-list-{i:03d}.json"
            if path.exists():
                path.unlink()
...
@pytest.mark.skipif(
    not (
        os.environ.get("CORTEXTERM_RUN_LIVE_API_TESTS") == "1"
        and os.environ.get("ANTHROPIC_API_KEY")
    ),
    reason=(
        "Set CORTEXTERM_RUN_LIVE_API_TESTS=1 and ANTHROPIC_API_KEY "
