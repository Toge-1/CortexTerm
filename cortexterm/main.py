...
from cortexterm.cli_commands import try_handle_local_command
from cortexterm.config import get_env, load_runtime_config
from cortexterm.history import load_history_entries, save_history_entries
from cortexterm.local_tool_shortcuts import parse_local_tool_shortcut
from cortexterm.manage_cli import maybe_handle_management_command
from cortexterm.mock_model import MockModelAdapter
...
def _configure_stdio() -> None:
    """Make startup output robust on Windows consoles and redirected pipes."""
...
def _render_banner(runtime: dict | None, cwd: str, permission_summary: list[str], counts: dict[str, int]) -> str:
    ...
    lines = [
        "CortexTerm",
        "agentic terminal for local code",
        "",
        f"model       {model}",
        f"cwd         {cwd}",
        f"skills      {counts['skillCount']}",
        f"mcp         {counts['mcpCount']}",
        f"transcript  {counts['transcriptCount']}",
    ]
    if permission_summary:
        lines.append("")
        lines.append("permissions")
        lines.extend(f"  {perm}" for perm in permission_summary[:3])
    return "\n".join(lines)


def _render_quick_start() -> str:
    """Render a compact startup guide for non-interactive mode."""
    return """Quick start:
  Read code:      ask about files, symbols, or project structure
  Edit files:     ask for a concrete change or bug fix
  Run commands:   /cmd <command> or ask to run tests
  Search code:    /grep <pattern> or describe what to find
  Tools:          /tools
  Help:           /help
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="CortexTerm - A lightweight terminal coding assistant",
        add_help=True,
    )
    parser.add_argument(
        "--resume",
    ...
    try:
        ...
        print(
            ...
            "  3. Or edit ~/.cortexterm/settings.json:\n"
             '     {"model": "claude-sonnet-4-20250514", "env": {"ANTHROPIC_API_KEY": "sk-ant-..."}}\n'
            "  4. Restart CortexTerm\n\n"
            "For more info: https://github.com/Toge-1/CortexTerm\n"
    ...
    model = (
        MockModelAdapter()
        if runtime is None or get_env("CORTEXTERM_MODEL_MODE") == "mock"
    ...
    from cortexterm.logging_config import get_logger
    ...
    try:
        if not sys.stdin.isatty():
            for raw_input in sys.stdin:
                ...
                def on_context_event(event: str, payload: dict) -> None:
                    if event == "compact_start":
                        ...
                        body = (
                            f"Context compaction started. {usage:.0f}% "
                            f"({before:,}/{window:,} tokens)."
                            if window
                            else "Context compaction started."
                        )
                    elif event == "compact_done":
                        before = int(payload.get("before_tokens") or 0)
                        after = int(payload.get("after_tokens") or 0)
                        usage = float(payload.get("usage_percentage") or 0)
                        body = f"Context compaction complete. {before:,} -> {after:,} tokens; now {usage:.0f}%."
                    else:
                        body = f"Context event: {event}"
                    _append_transcript(transcript, kind="progress", body=body)
                    print(body)
