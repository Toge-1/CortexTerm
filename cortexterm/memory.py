...
- User memory (~/.cortexterm/memory/) - cross-project, persistent
- Project memory (.cortexterm-memory/) - shared across sessions, can be versioned
- Local memory (.cortexterm-memory-local/) - project-specific, not checked in
...
class MemoryScope(str, Enum):
    """Memory scope levels."""
    USER = "user"       # Cross-project, ~/.cortexterm/memory/
    PROJECT = "project" # Project-shared, .cortexterm-memory/
    LOCAL = "local"     # Project-local, .cortexterm-memory-local/
...
class MemoryFile:
    ...
    def update_entry(self, entry_id: str, content: str) -> bool:
        ...
        for entry in self.entries:  # 遍历所有条目
            if entry.id == entry_id:  # 找到匹配的条目
                entry.content = content  # 更新内容
                entry.updated_at = time.time()  # 更新时间戳
                return True  # 返回成功
        return False  # 未找到条目，返回失败
...
class MemoryPaths:
    ...
    def for_workspace(cls, workspace: str) -> "MemoryPaths":
        ...
        return cls(
            user_memory=CORTEXTERM_DIR / "memory",
            project_memory=workspace_path / ".cortexterm-memory",
            local_memory=workspace_path / ".cortexterm-memory-local",
        )
...
class MemoryManager:
    ...
    def get_relevant_context(
        ...
        """
        from cortexterm.context_manager import estimate_tokens
