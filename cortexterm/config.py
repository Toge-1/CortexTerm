...
def _get_effective_env(env: dict[str, Any], name: str) -> Any:
    process_value = get_env(name)
...
def load_effective_settings(cwd: str | Path | None = None) -> dict[str, Any]:
    ...
    global_mcp = read_mcp_config_file(CORTEXTERM_MCP_PATH)
    project_mcp = read_mcp_config_file(project_mcp_path(cwd))
    cortexterm_settings = read_settings_file(CORTEXTERM_SETTINGS_PATH)
...
def save_cortexterm_settings(updates: dict[str, Any]) -> None:
    CORTEXTERM_DIR.mkdir(parents=True, exist_ok=True)
    existing = read_settings_file(CORTEXTERM_SETTINGS_PATH)
    next_settings = merge_settings(existing, updates)
    CORTEXTERM_SETTINGS_PATH.write_text(
...
def load_runtime_config(cwd: str | Path | None = None) -> dict[str, Any]:
    ...
    deepseek_api_key = str(
        env.get("DEEPSEEK_API_KEY") or _read_windows_env_var("DEEPSEEK_API_KEY") or ""
    ).strip() or None
    if deepseek_api_key:
        model = _get_effective_env(env, "CORTEXTERM_MODEL") or "deepseek-v4-pro"
        base_url = "https://api.deepseek.com/anthropic"
        auth_token = None
        api_key = deepseek_api_key
    else:
        model = (
            _get_effective_env(env, "CORTEXTERM_MODEL")
    ...
    raw_max_output_tokens = (
        _get_effective_env(env, "CORTEXTERM_MAX_OUTPUT_TOKENS")
    ...
    if raw_max_output_tokens is not None:
        try:
            ...
            max_output_tokens = None
    if deepseek_api_key and max_output_tokens is None:
        max_output_tokens = 4096

    if not model:
        raise RuntimeError("No model configured. Set ~/.cortexterm/settings.json or ANTHROPIC_MODEL.")
    ...
    return {
        ...
        "sourceSummary": (
            ...
            f"{CLAUDE_SETTINGS_PATH}; process.env overrides"
        ),
    }


def get_mcp_config_path(scope: str, cwd: str | Path | None = None) -> Path:
    return project_mcp_path(cwd) if scope == "project" else CORTEXTERM_MCP_PATH
...
def validate_config(cwd: str | Path | None = None) -> tuple[bool, list[str]]:
    ...
    try:
        ...
        if "No model configured" in error_msg:
            suggestion = _suggest_model_name(get_env("CORTEXTERM_MODEL") or "")
            help_msg = (
                f"Error: {error_msg}\n\n"
                "How to fix:\n"
                "  1. Set model name: export ANTHROPIC_MODEL=claude-sonnet-4-20250514\n"
                "  2. Or edit ~/.cortexterm/settings.json:\n"
