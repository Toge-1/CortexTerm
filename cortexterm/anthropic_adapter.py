...
from cortexterm.config import get_env
from cortexterm.types import AgentStep, StepDiagnostics
...
def _get_retry_limit() -> int:
    try:
        value = int(float(get_env("CORTEXTERM_MAX_RETRIES") or DEFAULT_MAX_RETRIES))
    except ValueError:
        value = DEFAULT_MAX_RETRIES
    return max(0, value)


def _get_request_timeout_seconds(runtime: dict[str, Any]) -> float:
    raw_value = (
        get_env("CORTEXTERM_REQUEST_TIMEOUT_SECONDS")
        or runtime.get("requestTimeoutSeconds")
        or 60
    )
    try:
...
def _to_anthropic_messages(messages: list[dict[str, Any]]) -> tuple[str, list[dict[str, Any]]]:
    ...
    for message in messages:
        ...
        if role == "assistant_tool_call":
            for thinking_block in message.get("thinkingBlocks", []):
                if isinstance(thinking_block, dict):
                    _push_anthropic_message(converted, "assistant", dict(thinking_block))
...
class AnthropicModelAdapter:
    ...
    def next(self, messages: list[dict[str, Any]]) -> AgentStep:
        ...
        request_timeout = _get_request_timeout_seconds(self.runtime)
        response = None
        for attempt in range(max_retries + 1):
            try:
                response = urllib.request.urlopen(request, timeout=request_timeout)  # noqa: S310
                break
        ...
        return AgentStep(type="assistant", content=parsed_text, thinkingBlocks=thinking_blocks, kind=kind, diagnostics=diagnostics)
