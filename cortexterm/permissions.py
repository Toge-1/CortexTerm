...
from cortexterm.config import CORTEXTERM_PERMISSIONS_PATH
...
def _read_permission_store() -> dict[str, Any]:
    if not CORTEXTERM_PERMISSIONS_PATH.exists():
        return {}
    try:
        data = json.loads(CORTEXTERM_PERMISSIONS_PATH.read_text(encoding="utf-8"))
...
def _write_permission_store(store: dict[str, Any]) -> None:
    ...
    CORTEXTERM_PERMISSIONS_PATH.parent.mkdir(parents=True, exist_ok=True)
    
    # 写入临时文件
    fd, tmp_path = tempfile.mkstemp(
        dir=CORTEXTERM_PERMISSIONS_PATH.parent,
        suffix=".tmp"
    )
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            ...
            f.write('\n')
        # 原子替换
        os.replace(tmp_path, CORTEXTERM_PERMISSIONS_PATH)
    except Exception:
        # 清理临时文件
        try:
            os.unlink(tmp_path)
...
class PermissionManager:
    ...
    def ensure_path_access(self, target_path: str, intent: str) -> None:
        ...
        if self.prompt is None:
            raise RuntimeError(
                f"Path {normalized_target} is outside cwd {self.workspace_root}. Start cortexterm in TTY mode to approve it."
            )
        ...
        result = self.prompt(
            {
                "kind": "path",
                "summary": f"cortexterm wants {intent.replace('_', ' ')} access outside the current cwd",
                "details": [
    ...
    def ensure_command(
        ...
        if signature in self.session_allowed_commands or signature in self.allowed_command_patterns:
            return
        if self.prompt is None:
            raise RuntimeError(f"Command requires approval: {signature}. Start cortexterm in TTY mode to approve it.")
        # Distinguish forced prompts (external trigger) from dangerous commands
        summary = (
            "cortexterm wants to run a dangerous command"
            if not force_prompt_reason
            else "cortexterm wants approval for this command"
        )
        result = self.prompt(
            {
                "kind": "command",
    ...
    def ensure_edit(self, target_path: str, diff_preview: str) -> None:
        ...
        if (
            ...
            return
        if self.prompt is None:
            raise RuntimeError(f"Edit requires approval: {normalized_target}. Start cortexterm in TTY mode to review it.")
        result = self.prompt(
            {
                "kind": "edit",
                "summary": "cortexterm wants to apply a file modification",
