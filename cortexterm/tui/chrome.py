...
def use_ascii_ui() -> bool:
    """Return whether chrome should avoid Unicode decoration."""
    if _env_truthy("CORTEXTERM_UNICODE_UI"):
        return False
    if _env_truthy("CORTEXTERM_ASCII_UI"):
        return True
    return sys.platform == "win32"


def use_panel_frames() -> bool:
    """Return whether panels should draw full surrounding boxes."""
    return _env_truthy("CORTEXTERM_PANEL_FRAMES")


_ASCII_UI = use_ascii_ui()
...
def panel_row(left: str, width: int, right: str | None = None, border_color: str = "") -> str:
    ...
    if right:
        ...
        if gap < 1:
            ...
            gap = 1
        return f"{bc}{side}{RESET} {left}{' ' * gap}{right} {bc}{side}{RESET}"
    else:
        return f"{bc}{side}{RESET} {pad_plain(left, inner_width)} {bc}{side}{RESET}"
...
def render_panel(
    ...
    if not border_color:
        ...
        if "workspace" in title_lower or "cortexterm" in title_lower:
    ...
    if not use_panel_frames():
        if right_display:
            left_w = string_display_width(title_display)
            right_w = string_display_width(right_display)
            gap = max(1, width - left_w - right_w)
            header = f"{title_display}{' ' * gap}{right_display}"
        else:
            header = title_display
        ...
        if max_body_lines is not None:
            wrapped_lines = wrapped_lines[:max_body_lines]
    ...
    inner = width - 4
    divider_line = f"{BORDER_DIM}{ICON_DIVIDER * inner}{RESET}"
