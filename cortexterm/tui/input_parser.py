...
ParsedInputEvent = Union[KeyEvent, TextEvent, WheelEvent, MouseEvent]
...
def parse_escape_sequence(chunk: str) -> tuple[ParsedInputEvent | None, int]:
    ...
    if sgr_match:
        ...
        x = int(sgr_match.group(2))
        y = int(sgr_match.group(3))
        final = sgr_match.group(4)
        ...
        action = 'up' if final == 'm' else 'down'
        return MouseEvent(button=button_name, action=action, x=x, y=y), sgr_match.end()
