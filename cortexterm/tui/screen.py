...
# Strategy: use ?1000h (basic click + wheel) + ?1006h (SGR coordinates).
# Avoid ?1003h: it emits mouse-move events continuously and makes this manual
# renderer feel hot/laggy without a real hover component model.
ENABLE_MOUSE_TRACKING = "\u001b[?1000h\u001b[?1006h"
DISABLE_MOUSE_TRACKING = "\u001b[?1006l\u001b[?1000l"

# Terminal types that do not support alternate screen or mouse tracking on
# POSIX-like terminals.  Do not include "" for Windows: PowerShell/Windows
# Terminal commonly leave TERM unset even when VT alternate-screen and mouse
...
def _is_dumb_terminal() -> bool:
    ...
    if (get_env("CORTEXTERM_NO_ALT_SCREEN") or "").strip().lower() in {"1", "true", "yes", "on"}:
        return True

    term = os.environ.get("TERM", "").strip().lower()
    if sys.platform == "win32":
        return term == "dumb"
    return term in _DUMB_TERMS
