"""Interactive installer for CortexTerm.
...
def _install_launcher_script() -> str | None:
    ...
    if sys.platform == "win32":
        # Windows: Use ~/.cortexterm/bin with .bat script
        target_bin_dir = CORTEXTERM_DIR / "bin"
        launcher_path = target_bin_dir / "cortexterm.bat"
        python_exe = sys.executable.replace("/", "\\")
        launcher_script = "\r\n".join([
            "@echo off",
            "REM CortexTerm Launcher for Windows",
            f'"{python_exe}" -m cortexterm.main %*',
            "",
        ])
        launcher_command = "cortexterm.bat"
    elif sys.platform == "darwin":
        # macOS: Use ~/.local/bin with bash script (also works with zsh)
        target_bin_dir = home / ".local" / "bin"
        launcher_path = target_bin_dir / "cortexterm"
        python_exe = sys.executable
        launcher_script = "\n".join([
            "#!/usr/bin/env bash",
            "# CortexTerm Launcher for macOS",
            "# Works with bash, zsh, and other shells",
            "set -euo pipefail",
            f'exec "{python_exe}" -m cortexterm.main "$@"',
            "",
        ])
        launcher_command = "cortexterm"
    else:
    ...
    try:
        ...
        print(f"\n⚠️  无法安装启动器脚本: {e}")
        print("你可以手动创建启动器脚本来调用 cortexterm。")
        return None
...
def main() -> None:
    ...
    print("=" * 60)
    print("  CortexTerm 安装向导")
    print("=" * 60)
    print()
    print(f"配置会写入: {CORTEXTERM_SETTINGS_PATH}")
    ...
    try:
        ...
        print(f"✅ 配置已保存到: {CORTEXTERM_SETTINGS_PATH}")
    except OSError as e:
        print(f"\n❌ 保存配置失败: {e}")
        sys.exit(1)
    ...
    print("=" * 60)
    print()
    print("📁 配置文件:", CORTEXTERM_SETTINGS_PATH)
    if launcher_result:
    ...
    print()
    print("  Windows:")
    print("    cortexterm.bat               (如果已添加 PATH)")
    print("    python -m cortexterm.main    (通用方式)")
    print()
    print("  macOS:")
    print("    cortexterm                (如果已添加 PATH)")
    print("    python3 -m cortexterm.main   (通用方式)")
    print()
    print("  Linux:")
    print("    cortexterm                (如果已添加 PATH)")
    print("    python3 -m cortexterm.main   (通用方式)")
    print()
    print("感谢使用 CortexTerm！🎉")
    print()
