...
from cortexterm.poly_commands import create_builtin_commands, CommandRegistry
from cortexterm.config import (
    CLAUDE_SETTINGS_PATH,
    CORTEXTERM_MCP_PATH,
    CORTEXTERM_PERMISSIONS_PATH,
    CORTEXTERM_SETTINGS_PATH,
    load_runtime_config,
    save_cortexterm_settings,
)
...
SLASH_COMMANDS = [
    ...
    SlashCommand("/history", "/history", "Show recent prompt history from ~/.cortexterm/history.json."),
    ...
    SlashCommand("/model", "/model <model-name>", "Persist a model override into ~/.cortexterm/settings.json."),
    SlashCommand("/config-paths", "/config-paths", "Show cortexterm and Claude fallback settings paths."),
    ...
    SlashCommand("/permissions", "/permissions", "Show cortexterm permission storage path."),
    SlashCommand("/exit", "/exit", "Exit cortexterm."),
...
def try_handle_local_command(user_input: str, tools=None) -> str | None:
    ...
    if user_input == "/config-paths":
        return "\n".join(
            [
                f"cortexterm settings: {CORTEXTERM_SETTINGS_PATH}",
                f"cortexterm permissions: {CORTEXTERM_PERMISSIONS_PATH}",
                f"cortexterm mcp: {CORTEXTERM_MCP_PATH}",
                f"compat fallback: {CLAUDE_SETTINGS_PATH}",
            ]
        )

    if user_input == "/permissions":
        return f"permission store: {CORTEXTERM_PERMISSIONS_PATH}"

    if user_input == "/skills":
        skills = tools.get_skills() if tools else []
        if not skills:
            return "No skills discovered. Add skills under ~/.cortexterm/skills/<name>/SKILL.md, .cortexterm/skills/<name>/SKILL.md, .claude/skills/<name>/SKILL.md, or ~/.claude/skills/<name>/SKILL.md."
        return "\n".join(
    ...
    if user_input == "/context":
        ...
        try:
            from cortexterm.context_manager import load_context_state
    ...
    if user_input == "/mcp":
        ...
        if not servers:
            return "No MCP servers configured. Add mcpServers to ~/.cortexterm/settings.json, ~/.cortexterm/mcp.json, or project .mcp.json."
        lines = []
    ...
    if user_input.startswith("/model "):
        ...
        save_cortexterm_settings({"model": model})
        return f"saved model={model} to {CORTEXTERM_SETTINGS_PATH}"
