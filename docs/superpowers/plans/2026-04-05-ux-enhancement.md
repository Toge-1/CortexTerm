...
**目标：** 优化 CortexTerm 版用户交互体验，降低学习成本，提升使用流畅度
...
- 修改：`cortexterm/tty_app.py` - 添加上下文帮助和状态提示
- 修改：`cortexterm/main.py` - 增强启动引导
- 修改：`cortexterm/cli_commands.py` - 优化帮助信息
- 创建：无新文件，遵循现有代码模式
...
**文件：**
- 修改：`cortexterm/tty_app.py:300-320`
...
**文件：**
- 修改：`cortexterm/tui/chrome.py:render_footer_bar`
...
**文件：**
- 修改：`cortexterm/tty_app.py:on_tool_start, on_tool_result`
...
**文件：**
- 修改：`cortexterm/tty_app.py:on_tool_result`
...
- [ ] **步骤 1：运行现有测试**
```bash
cd D:\Desktop\cortexterm\py-src && python -m pytest tests/ -v
```
